# Bypass the execution policy for this session only to run the script
Set-ExecutionPolicy -ExecutionPolicy Bypass -Scope Process -Force

Write-Host "[+] Starting deep system scan for EXACT malicious package versions..." -ForegroundColor Green
Write-Host "[+] This will take a significant amount of time. Please wait." -ForegroundColor Yellow

# Define the malicious package:version pairs as a hashtable for fast lookup
$MaliciousPackages = @{
    "ansi-styles"                                   = @("6.2.2")
    "debug"                                         = @("4.4.2")
    "chalk"                                         = @("5.6.1")
...
# Find all package-lock.json files on the C: drive
$LockFiles = Get-ChildItem -Path C:\ -Name "package-lock.json" -File -Recurse -ErrorAction SilentlyContinue | ForEach-Object { "C:\$_" }

Write-Host "[+] Found $($LockFiles.Count) package-lock.json files to scan." -ForegroundColor Cyan
...
foreach ($LockFile in $LockFiles) {
    $Counter++
    Write-Progress -Activity "Scanning Package-Lock Files" -Status "Scanning: $LockFile" -PercentComplete (($Counter / $LockFiles.Count) * 100)

    try {
        # Read the lockfile content and convert from JSON
        $JsonContent = Get-Content -Path $LockFile -Raw | ConvertFrom-Json -ErrorAction Stop

        # Check dependencies
        if ($JsonContent.PSObject.Properties.Name -contains "dependencies") {
            foreach ($PkgName in $JsonContent.dependencies.PSObject.Properties.Name) {
                if ($MaliciousPackages.ContainsKey($PkgName)) {
                    $InstalledVersion = $JsonContent.dependencies.$PkgName.version
                    if ($MaliciousPackages[$PkgName] -contains $InstalledVersion) {
                        $Result = [PSCustomObject]@{
                            FilePath        = $LockFile
                            PackageName     = $PkgName
                            BadVersion      = $InstalledVersion
                            Type            = "dependencies"
                        }
                        $Results += $Result
                    }
        ...
        if ($JsonContent.PSObject.Properties.Name -contains "devDependencies") {
            foreach ($PkgName in $JsonContent.devDependencies.PSObject.Properties.Name) {
                if ($MaliciousPackages.ContainsKey($PkgName)) {
                    $InstalledVersion = $JsonContent.devDependencies.$PkgName.version
                    if ($MaliciousPackages[$PkgName] -contains $InstalledVersion) {
                        $Result = [PSCustomObject]@{
                            FilePath        = $LockFile
                            PackageName     = $PkgName
        ...
        # Silently skip files that can't be parsed (e.g., corrupt or not JSON)
    }
}

Write-Progress -Activity "Scanning Package-Lock Files" -Completed

# Display the results
if ($Results.Count -gt 0) {
    Write-Host "`n[!!!] CRITICAL: Found $($Results.Count) infected dependencies!" -ForegroundColor Red
    $Results | Format-Table -AutoSize
    Write-Host "`nIMMEDIATE ACTION REQUIRED:" -ForegroundColor Red
    Write-Host "1. DELETE the 'node_modules' folder in each project listed above." -ForegroundColor Yellow
    Write-Host "2. DELETE the 'package-lock.json' file." -ForegroundColor Yellow
    Write-Host "3. EDIT 'package.json' to pin a safe version of the package." -ForegroundColor Yellow
    Write-Host "4. Run 'npm install --ignore-scripts' from Command Prompt." -ForegroundColor Yellow
    Write-Host "5. ROTATE ALL SECRETS (NPM_TOKEN, GITHUB_TOKEN, AWS keys, etc.)." -ForegroundColor Red
} else {
    Write-Host "`n[+] Scan complete. No exact malicious package versions were found." -ForegroundColor Green
    Write-Host "[+] Note: This is excellent news, but you should still rotate secrets as a precaution." -ForegroundColor Yellow
}

# Revert the execution policy for this session back to Restricted
